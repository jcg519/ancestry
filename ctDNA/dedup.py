...
def parseCommand():
    usage = "usage: ./dedup.py -1 /haplox/rawout/150626/150702/liliang_cfDNA-009_sort.bam -o /haplox/users/huang/mypy/liliang_cfDNA-009_sort_test.txt > liliang_cfDNA-009_sort_test_len.dedup"
    version = "%prog 1.0"
    parser = OptionParser(usage = usage, version = version)
    parser.add_option("-1", "--input1", dest = "input1", help = "the sam file")
    parser.add_option("-q", "--phredQ", dest = "phred", help = "Phred Q", type="int", default="20" )
    parser.add_option("-o", "--output", dest = "output", default = "/haplox/users/fastq", help = "输出目录".decode("utf8"))
...
def block_compare(forward_block_dict_list, reverse_block_dict):
    dup_seqname_dict = {}
    save_number_dict = {}
        ...
        for j in range((i+1), len(forward_block_dict_list)):
            if forward_block_dict_list[j][1] not in dup_seqname_dict:
                if forward_block_dict_list[j][1] in reverse_block_dict:
                    if str_compare(forward_block_dict_list[i], forward_block_dict_list[j], reverse_block_dict[forward_block_dict_list[i][1]], reverse_block_dict[forward_block_dict_list[j][1]]):
        ...
        if frag.is_proper_pair:
            if frag.template_length > 0:
                if frag.reference_id not in forward_block_dict:
                    ...
                    forward_block_dict[frag.reference_id][frag.reference_start]=[[total, frag.query_name, frag.query_sequence, frag.query_qualities]]
                    if not chrom_flag:
                    ...
                    for i in forward_block_dict[chrom_tmp].keys():
                        if len(forward_block_dict[chrom_tmp][i]) >= 2:
                            line_list = block_compare(forward_block_dict[chrom_tmp][i], reverse_block_dict[chrom_tmp])
                    ...
                    forward_block_dict[frag.reference_id][frag.reference_start].append([total, frag.query_name, frag.query_sequence, frag.query_qualities])
            elif frag.template_length < 0:
                if frag.reference_id not in reverse_block_dict:
                    reverse_block_dict[frag.reference_id]={}
                    reverse_block_dict[frag.reference_id][frag.query_name] = [total, frag.query_name, frag.query_sequence, frag.query_qualities]
                else:
                    reverse_block_dict[frag.reference_id][frag.query_name] = [total, frag.query_name, frag.query_sequence, frag.query_qualities]
            else:
